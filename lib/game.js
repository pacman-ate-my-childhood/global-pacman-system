...
Game.prototype.join = function(user, callback) {
   if (!this.users[user.id]) { // if not already in the game
      if (this.avaliable_characters.length > 0) {
         ...
         this.users[user.id] = character.name;

         this.save(function(err, game) {
            if (err) callback(err);
            else {
               console.log('emiting a join message u:' + user.id + ' c:' + character.name);
               Channel.get(game.id).emit('join', { username: user.id, character: character.name });
               callback(err, game);
            }
         });
...
Game.prototype.leave = function(user, callback) {
   var character = this.users[user.id];

   if (character) { // if the user has a character in this game
      this.avaliable_characters.push(character);
      ...
      if (this.avaliable_characters.length === MAX_PLAYERS) { // the game is now empty
         ...
         this.save(function(err, game) {
            if (err) callback(err);
               ...
               Channel.get(game.id).emit('leave', { username: user.id, character: character.name });
               callback(err, game);
            }
         });
      }
   } else {
      callback('user, ' + user.id + ', is not a part of this game', this);
   }
};
   ...
   channel.on('connection', function(socket) {
      socket.on('move', function(data) {
         ...
         Game.get(id, function(err, game){
            ...
            var character = game.characters[game.users[data.username]];
