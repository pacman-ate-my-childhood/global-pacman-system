...
Map.create = function(params, callback) {
   params.centre = _.reduce(params.vertices, function(centre, vertex, num, vertices) { 
...
Map.list = function(callback) {
   map_hash.values(function(err, maps) {
      if (err) { callback(err); return; }

      callback(err, _.map(_.map(maps, JSON.parse), function(map) { return new Map(map.id, map); }));
   });
};
